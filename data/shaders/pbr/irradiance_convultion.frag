...
void main() {
    ...
    for(float phi = 0.0; phi < 2.0 * PI; phi += sampleDelta)
    {
        for(float theta = 0.0; theta < 0.5 * PI; theta += sampleDelta)
        {
            ...
            vec3 sampleEnv = texture(environmentMap, sampleVec).rgb;
            irradiance += sampleEnv * cos(theta) * sin(theta);
            nrSamples++;
        }
    }
    // on divise par le nombre de sample car on enlève le biais de l'approximation.
